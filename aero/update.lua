...
--
-- Ships can also be updated over the wire: the pocket computer's fleet screen
-- sends this. A ship that is airborne refuses -- see pilot.lua -- because
-- replacing lib/hull.lua underneath a live control loop, and then rebooting,
-- means several seconds during which nothing at all is holding the ship up.
--
...
-- half-finished update never happens. A tree where pilot.lua is new and
-- lib/hull.lua is old fails in ways that look like bugs in neither.
