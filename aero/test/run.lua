...
-- fly_spec.lua follows it: it is the one that asks whether the thing flies, and
-- it depends on nothing but the pure modules and the mock.
--
...
local suites = {
  ...
  { name = "fly",     path = "/test/fly_spec.lua"    },
  { name = "pilot",   path = "/test/pilot_spec.lua"  },
...
for _, suite in ipairs(suites) do
  local totals = _G.AERO_TOTALS[suite.name]
  if totals then
