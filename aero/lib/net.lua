...
-- can set.
--
-- This is the same design as mining/lib/net.lua and redstone/lib/net.lua, and
-- deliberately a copy rather than a shared root library: each directory installs
-- and updates on its own, and a shared module would mean updating the redstone
-- network rebooting every ship in the air.
--
...
--
-- Wireless only. A wired modem would work for messaging between two computers
-- bolted to the same hull, but the whole point of the roster is that the pocket
-- computer in your hand can reach a ship that is half a kilometre away and four
-- hundred blocks up, and a wired modem cannot do that.
--
-- Note this is also the modem gps.locate() uses, which is why lib/instruments
-- can ask for a GPS fix without any hardware of its own.
...
local function transmit(to, message)
  ...
  net.modem.transmit(net.channel, net.channel, {
    aero = config.protocol,      -- see below
    from = net.id,
    to   = to,
    body = message,
  })
...
--- not for us. Split out because every program here runs its own event loop --
--- the pilot has a control loop to run, the other two have a UI to redraw -- and
--- none of them can sit blocked inside net.receive.
--
...
-- where a ship quietly takes orders from the wrong tower.
function net.decode(event, side, channel, reply, frame)
  ...
  if frame.aero ~= config.protocol then return nil end
  ...
  -- that, and a server that folded its own telemetry into the roster would be
  -- tracking a ship that does not exist.
